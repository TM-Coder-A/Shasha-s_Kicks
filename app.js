document.addEventListener("DOMContentLoaded", async () => {
  ...
  renderManagedProducts(products);
  addProductNamesToContactForm(products);
});
...
function applySiteSettings() {
  ...
  if (footerBrand) {
    footerBrand.innerHTML = `© <span id="year">${new Date().getFullYear()}</span> ${skEscape(settings.brandName)}. All rights reserved.`;
  }
...
function renderManagedProducts(products) {
  const grid =
    document.getElementById("managedProductGrid") ||
    ...
    return `
      <article class="managed-product-card">
        ...
        <div class="managed-product-body">
          ...
          <small><b>Sizes:</b> ${skEscape(product.sizes || "Available on request")}</small>
...
function addProductNamesToContactForm(products) {
